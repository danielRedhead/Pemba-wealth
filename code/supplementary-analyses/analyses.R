...
ind <- data.frame(Wealth = log(su$su_direct_wealth))

group_ids <- data.frame(Status = as.factor(su$khazi))
...
in_fit <- fit_latent_network_model( data=dat,
      fpr_regression = ~ 1, 
      rtt_regression = ~ 1, 
      theta_regression = ~ 1, 
      focal_regression = ~ 1, 
...
# Extract results
in_res <- summarize_strand_results(in_fit)

in_out_fit <- fit_latent_network_model( data=dat,
      fpr_regression = ~ 1, 
      rtt_regression = ~ 1, 
      theta_regression = ~ 1, 
      focal_regression = ~ Wealth, 
      target_regression = ~ Wealth, 
      dyad_regression = ~ Relatedness + Distance + Female_k + Female_q + Male_k + Male_q,
      block_regression = ~ Status,
       mode="mcmc",
       return_latent_network = TRUE, 
       stan_mcmc_parameters = list(chains = 1, parallel_chains = 1, refresh = 1,
...
# Extract results
in_out_res <- summarize_strand_results(in_out_fit)
...
save.image(file = "./3-analyses/results.RData")
